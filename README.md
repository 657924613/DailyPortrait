...
UI Layer
  ...
  Dashboard / Camera / Settings / VideoExport

Domain Layer
  PhotoRepository (interface) / GenerateVideoUseCase
  StreakCalculator / DateUtil / BitmapUtil

Data Layer
  Room DAO / FileManager / FaceAnalyzer (ML Kit)
...
- ML Kit 每帧分析人脸 BoundingBox（小于 30ms）
- 归一化坐标（0 到 1）跨分辨率复用
...
- 支持 4 档画质（720p 到 4K）+ 帧率 2 到 20fps
...
```bash
git clone https://github.com/657924613/DailyPortrait.git
...
[点击下载最新版 APK](https://github.com/657924613/DailyPortrait/releases/latest/download/app-debug.apk)

安装方式：下载后传到 Android 手机，打开安装即可（需开启"允许安装未知来源"）。

也可以在 [Releases 页面](https://github.com/657924613/DailyPortrait/releases) 查看所有历史版本。
...
| [架构说明书](architecture-android.md) | Android 技术架构 + 分步实现指令 |
...
这个项目从设计文档分析到 3 套 HTML 原型到 Android 原生实现到系统性优化，全程由 AI 辅助完成（Kiro + Claude），历时约 2 天。
